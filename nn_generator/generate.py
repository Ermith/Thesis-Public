from libraries.matrix_manipulation import rescale, cutout, derivatives
from libraries.images import save_image, init_cmaps, save_all
from libraries.parsing import get_window
from libraries.utilities import noisify_exp, unary_log_encoding_array, unary_log_decoding, relativization, unary_linear_encoding,  unary_log_encoding_array_reversed, unary_log_decoding_reversed
...
def load_data(model_file, data_folder, result_folder, data_name):
    ...
    image = cv2.imread(f"{data_folder}/{data_name}.pgm", cv2.IMREAD_GRAYSCALE)
    image = rescale(image)
...
def generate_map(generation_data, results_folder, config_file="generation_config.txt"):
    ...
    heights = noisify_exp(heights, 20)
    heights = rescale(heights)
    heights = 1 - heights
    heights = np.pad(heights, padding, 'reflect')
    ...
    generated_heights_16x = noisify_exp(generated_heights_16x, 20)
    ...
    generated_heights_4x = noisify_exp(generated_heights_4x, 20)
